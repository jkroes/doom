...
;; The documentation is still under development as part of Doom v3. The docs
;; are fairly oudated; e.g., they still refer in some places to the old
...
;; again.
;;
;; If an error appears when launching Emacs via Hammerspoon, try replicating it
;; after launching Emacs from the shell. Some errors disappear when launching
;; from the shell, despite the presence of an env file generated by doom env.
;; One example of this is running `find-file' when :pwd is ~/. An error may
;; be thrown by `vertico--exhibit', depending on the contents of ~/.Trash
;;
...
;; NOTE This is less useful for commands where Doom defines wk descriptions
...
(setq doom-font (font-spec
                 :family "Hack"
                 :size
                 (if (length= (display-monitor-attributes-list) 2) 18 14)))
...
(defadvice! my/default--newline-indent-and-continue-comments-a (&rest _)
  ...
  (when (and +default-want-RET-continue-comments
    ...
    ;; Call the original function when the cursor is preceded by whitespace (e.g.
    ;; before the first comment character)
    (unless (save-excursion (skip-chars-backward "\s") (= 0 (current-column)))
      ;; On an empty comment line, remove the comment characters but don't insert
      ;; a newline yet. Equivalent to pressing RET twice at the end of a non-empty
      ;; comment.
      (if (save-excursion (end-of-line)
          ...
          (progn
            ...
            t)
        ;; Call the original function when the cursor is within whitespace between
        ;; a comment and a nonspace character.
        (unless (save-excursion (skip-chars-backward
                                 (concat comment-start "\s"))
                                (= 0 (current-column)))
          ;; BUG When `comment-auto-fill-only-comments' is non-nil,
          ;; `comment-line-break-function' behaves insanely when the cursor is on a
          ;; blank emacs lisp comment line with any amount of space following the
          ;; comment characters. The misbehavior actually increases in proportion
          ;; to the number of spaces between the comment's start and the cursor. Even
          ;; if you want to limit auto-fill to comments, and even if you don't want
          ;; the rest of my changes to this advice above, you should still use this.
...
;; `+default/newline-below', and I need a way to continue comments beyond
;; simply from a nonblank comment line. So let's reuse the bindings for
;; `+default/newline-below'.NOTE `comment-indent-new-line' is the current value
;; of `comment-line-break-function'.
(map! :gi "C-RET"         #'comment-indent-new-line
      :gi [C-return]      #'comment-indent-new-line
      :gn "C-RET"         #'comment-indent-new-line
      :gn [C-return]      #'comment-indent-new-line)
...
;; top-level DIR property for org-roam notes.
(after! org
...
;; modules/lang/org/contrib/pretty.el makes a mistake. It is not enough to set
;; org-hide-leading-stars to nil; you need to make sure org-indent-mode, which
;; doom enables by default, does not enable this variable buffer locally.
(setq org-indent-mode-turns-on-hiding-stars nil)

;; TODO Customize org-superstar-todo-bullet-alist
;; Implements org-superstar-todo-bullet-alist
(after! org-superstar
...
;; TODO Integrate dendroam hierarchy and title with title given to citar
;; reference notes. Use aliases to bridge the gap if necessary. Maybe the main
;; title can be the last component of the hierarchy, while the alias can be
;; citar-org-roam-note-title-template... Brainstorm and play around with it.
...
;; Location of citar notes (what I call "reference" notes)
(setq citar-notes-paths (list org-directory))
;;(setq citar-notes-paths (list (concat-path org-directory "references")))

;; NOTE Windows Zotero can't write to org-directory (TODO OneDrive or WSL?)
(setq citar-bibliography
      (cond (IS-WSL (list "/mnt/d/org-cite.bib"))
...
;; NOTE Can you do the same thing with the :create key of
;; citar-org-roam-notes-config?
;;
;; Use a custom citar note creation function
(after! citar
  (setf (plist-get (alist-get 'citar-org-roam citar-notes-sources)
                   :create)
        'citar-org-roam--create-capture-note2))
...
;; TODO If you ever require citar-org-roam, you can replace `defvar' with `setq'
(defvar citar-org-roam-note-title-template "${title}")
...
(defun citar-org-roam--create-capture-note2 (citekey entry)
  ...
  (let ((title (citar-format--entry
    ...
    (apply 'org-roam-capture-
           ;; HACK Modifying this allows new variables (e.g., date) in template
           ;; strings
           :info (list :citekey citekey
                       :date (alist-get "date" entry nil nil #'string=))
...
;; TODO Don't reenable this, but do figure out why it doesn't seem to exclude
;; citar notes from the org-roam database
;;(push (file-name-as-directory (car citar-notes-paths)) org-roam-file-exclude-regexp)
