...
(use-package! vertico
  ...
  ;; `setq-default' ensures that this setting does not override corfu. Otherwise,
  ;; this allows for vertico-based completion on M-TAB. See
  ;; https://web.archive.org/web/20220914132011/https://with-emacs.com/posts/tutorials/customize-completion-at-point/
  ;; and the consult README. Apparently consult-completion-in-region doesn't work
  ;; with LSP.
...
(use-package! consult
  ...
  ;; BUG The default  module sets `consult-project-root-function', which doesn't exist
  ...
  (consult-customize
   ...
   :preview-key "C-SPC")
  ;; Preview themes after 0.5s idle after calling consult-theme
  (consult-customize
   consult-theme
...
;; NOTE Marginalia annotates minibuffer completions, but if a
;; minibuffer-completion command is not the top-level command executed, it
;; might not be annotated correctly. E.g. I modified +org/dwim-at-point to call
;; org-attach-open. To have these commands annotated correctly, you need to
;; either rebind this-command to the command you want annotated before
;; executing it, or else execute it via execute-extended-command; e.g.
